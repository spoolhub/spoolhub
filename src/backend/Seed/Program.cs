...
var dbPath = args.Length > 0 ? args[0] : Path.Combine("..", "API", "spoolhub.db");
var fullPath = Path.GetFullPath(dbPath, AppContext.BaseDirectory);

if (!File.Exists(fullPath))
{
    Console.Error.WriteLine($"Database not found: {fullPath}");
    Console.Error.WriteLine("Usage: dotnet run --project src/backend/Seed [path-to-spoolhub.db]");
    Environment.Exit(1);
}

var builder = new DbContextOptionsBuilder<FilamentDbContext>();
builder.UseSqlite($"Data Source={fullPath}");

using var db = new FilamentDbContext(builder.Options);

await db.Database.EnsureCreatedAsync();

if (db.Printers.Any())
{
    Console.WriteLine("Printers already exist — skipping seed.");
    return;
}
...
Console.WriteLine($"    6 locations (Shelf A1/A2, Shelf B1/B2, Drybox 1/2)");
Console.WriteLine($"   15 app settings (alerts, filaments, app defaults)");
Console.WriteLine($"    6 brands (Bambu Lab, eSUN, Prusament, Polymaker, Hatchbox, Overture)");
Console.WriteLine($"    8 spool profiles");
Console.WriteLine($"   13 spools (all fields: temps, density, weight, location, notes)");
Console.WriteLine($"    5 NFC tags");
Console.WriteLine($"    4 printers (X1 Carbon / P1S / A1 Mini / Prusa MK4) with protocol");
Console.WriteLine($"    7 print jobs (5 finished, 1 running, 1 failed) with LastUpdatedAt");
Console.WriteLine($"   10 print job filament records (AMS slot breakdown)");
Console.WriteLine($"   14 activities");
