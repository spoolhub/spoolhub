...
// Private apps with componentSrcMap and noEmit:true have no .d.ts so exportedNames()
// returns empty. Populate exported from the src map + provider so the provider
// wrap check (emit.mjs:310) and any cross-reference checks work correctly.
if (!src.synthEntry && Object.keys(cfg.componentSrcMap ?? {}).length) {
  for (const c of src.components) exported.add(c.name);
  if (PROVIDER?.component) exported.add(PROVIDER.component.split('.')[0]);
}
