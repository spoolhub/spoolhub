...
export default function Sidebar({ isOpen, onClose, spoolCount }: SidebarProps) {
  ...
  return (
    <aside className={`side${isOpen ? ' open' : ''}`}>
      {/* Collapse toggle (railfab) */}
      <button className="railfab" id="railToggle" onClick={toggleCollapsed} title={isCollapsed ? 'Expand sidebar' : 'Collapse sidebar'}>
      ...
      <div className="brand">
        <NavLink to="/" end onClick={onClose} style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
          ...
          <span className="brandName">Spool<b>Hub</b></span>
        </NavLink>
      ...
      <nav className={styles.nav}>
        ...
        <NavLink to="/spools" end onClick={onClose} className={navClass}>
          ...
          Spools
          {spoolCount !== undefined && <span className="badge">{spoolCount}</span>}
        </NavLink>
        ...
        <div className="usercard">
          ...
          <div className="who">
            <div className="n">Mira Kovač</div>
            <div className="e">studio · pro plan</div>
          </div>
        </div>
