...
import styles from './AddSpoolProfilePage.module.css'
...
export default function AddSpoolProfilePage() {
  ...
  if (saved && selectedFilament) {
    return (
      <div className={styles.wrap}>
        <div className={styles.card}>
          <div className={styles.cardHeader}>
            <div>
              <h1 className={styles.title}>{t('spoolProfile.successTitle')}</h1>
              <p className={styles.subtitle}>{selectedFilament.brand} · {selectedFilament.material} · {selectedFilament.colorName}</p>
            </div>
          ...
          <div className={styles.successBtns}>
            <button
              ...
              onClick={() => { setSaved(false); setSelectedFilament(null) }}
            >
              {t('spoolProfile.addAnother')}
            </button>
            <button
              type="button"
              className={styles.btnPrimary}
              onClick={() => navigate('/spool-profiles')}
            >
              {t('spoolProfile.viewProfiles')}
            </button>
          </div>
        </div>
      </div>
  ...
  if (selectedFilament) {
    return (
      <div className={styles.wrap}>
        <button type="button" className={styles.backBtn} onClick={() => setSelectedFilament(null)}>
        ...
        <form onSubmit={handleSubmit} className={styles.card}>
          ...
          <div className={styles.formActions}>
            <button type="button" className={styles.btnSecondary} onClick={() => setSelectedFilament(null)}>
              {t('common.cancel')}
            </button>
  ...
  return (
    <div className={styles.wrap}>
      <button type="button" className={styles.backBtn} onClick={() => navigate('/spool-profiles')}>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <polyline points="15 18 9 12 15 6" />
        </svg>
        {t('spoolProfile.backToProfiles')}
      </button>
