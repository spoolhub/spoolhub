...
import styles from './ActiveSpoolsPage.module.css'
...
export default function ActiveSpoolsPage() {
  ...
  const activeSpools = useMemo(
    () => spools.filter(s => s.isActive),
    [spools]
  )

  const allMaterials = useMemo(() => [...new Set(activeSpools.map(s => s.material))].sort(), [activeSpools])
  const allBrands    = useMemo(() => [...new Set(activeSpools.map(s => s.brand))].sort(), [activeSpools])
  const allColors    = useMemo(() => {
    const seen = new Map<string, string>()
    activeSpools.forEach(s => { if (!seen.has(s.colorHex)) seen.set(s.colorHex, s.colorName) })
  ...
  const filtered = useMemo(() => {
    return [...activeSpools]
      ...
      .sort((a, b) => a.currentWeightG - b.currentWeightG)
  }, [activeSpools, q, filters])
